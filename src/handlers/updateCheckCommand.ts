...
export async function handleUpdateCheckCommand(
  ...
  if (sub === 'notify') {
    ...
    await interaction.reply({
      content: `Automatic update alerts are now **${enable ? 'enabled' : 'disabled'}**.`,
  ...
  if (status.state === 'up_to_date') {
    await interaction.editReply(
      `Running latest commit (${shortSha(status.local)}). Auto alerts are **${getVersionNotifyEnabled() ? 'enabled' : 'disabled'}**.`,
    );
    return;
  }

  await interaction.editReply(
    `Update available: remote **${shortSha(status.remote)}** vs local **${shortSha(
      status.local,
    )}**.\nAuto alerts are **${getVersionNotifyEnabled() ? 'enabled' : 'disabled'}**. Use \`git pull && bun install\` and restart the bot when ready.`,
  );
